"""Train a generator and a discriminator architectures with a GAN loss."""
...
if __name__ == '__main__':
    ...
    lr_decay_epochs = cfg['training']['lr_decay_epochs']
    lr_decay_every = cfg['training']['lr_decay_every_n_steps']
    ...
    generator = misc.construct_class_by_name(
        class_name="models.SG2WaveletGenerator",
        z_dim=64,
        divide_channels_by=8,
        architecture="skip",
        upscale_bilinear=True,
        normalize_dwt=True,
        # z=dataset.z,
        **common_kwargs)
    ...
    if lr_decay_epochs > 0:
        g_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=g_optimizer, gamma=lr_decay_rate)
        d_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=d_optimizer, gamma=lr_decay_rate)
    else:
        g_lr_scheduler = None
        d_lr_scheduler = None
    # g_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=g_optimizer, gamma=lr_decay_rate)
    ...
    def discriminator_trainstep(x_real, z, cls_label, reg_param=10):
        ...
        d_real = model(x_real, c=cls_label)
        targets = d_real.new_full(size=d_real.size(), fill_value=1)
        if use_spec_disc:
            d_real_spec = spec_disc_step(x_real, c=cls_label)
        ...
        # print('DBG: x_fake.requires_grad =', x_fake.requires_grad)  # Prints True.
        x_fake.requires_grad_()

        d_fake = model(x_fake, c=cls_label)
        targets = d_fake.new_full(size=d_fake.size(), fill_value=0)
        if use_spec_disc:
            d_fake_spec = spec_disc_step(x_fake, c=cls_label)
    ...
    def generator_trainstep(z, cls_label):
    ...
    while epoch_idx < nepochs:
        ...
        for img, z, class_label in dataloader:
            it += 1
            img, z, class_label = img.to(device), z.to(device), class_label.to(device)

            if it > 0:      # only evaluate at initialization
                model = model.train()

                # Model updates
                dloss, reg = discriminator_trainstep(img, z, class_label, reg_param=cfg.training.reg_param)
                logger.add('losses', 'discriminator', dloss, it=it)
                logger.add('losses', 'regularizer', reg, it=it)

                # Image updates
                gloss = generator_trainstep(z, class_label)
            ...
            if lr_decay_epochs > 0 and (epoch_idx >= nepochs - 1 - lr_decay_epochs) and (it % lr_decay_every) == 0:
                g_lr_scheduler.step()
                d_lr_scheduler.step()
